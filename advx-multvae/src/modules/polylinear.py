...
class PolyLinear(nn.Module):
    def __init__(self, layer_config: list, activation_fn: Union[str, nn.Module] = nn.ReLU(),
                 output_fn=None, input_dropout=None, l1_weight_decay=None):
        """
        ...
        activation_fn = ACTIVATION_FN_MAP[activation_fn] if isinstance(activation_fn, str) else activation_fn
...
class PolyLinearParallel(nn.Module):
    def __init__(self, layer_config: list, n_parallel: int, activation_fn: Union[str, nn.Module] = nn.ReLU(),
                 output_fn=None, input_dropout=None, l1_weight_decay=None):
        super().__init__()

        self.parallel_layers = Parallel(parallel_mode=ParallelMode.SingleInMultiOut)
        for _ in range(n_parallel):
            self.parallel_layers.append(
                PolyLinear(layer_config, activation_fn, output_fn, input_dropout, l1_weight_decay))

    def forward(self, x):
        return self.parallel_layers(x)
